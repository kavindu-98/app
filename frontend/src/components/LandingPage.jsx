...
import { concertData, bookingData, bookTickets } from '../data/mock';
...
const LandingPage = () => {
  ...
  const handleBooking = async (ticketType) => {
    try {
      const response = await bookTickets({ ticketType, quantity: 1 });
      if (response.success) {
        toast({
            ...
            <div className={`transition-all duration-1000 delay-400 ${
              ...
              <div className="bg-gradient-to-br from-amber-50 to-white rounded-2xl p-8 shadow-xl border border-amber-100">
                ...
                <div className="space-y-4 mb-8">
                  {bookingData.ticketTypes.map((ticket, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-white rounded-lg border border-amber-200">
                      <div>
                        <h4 className="font-semibold text-gray-800">{ticket.type} Seating</h4>
                        <p className="text-gray-600">{ticket.available} seats available</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-amber-600 text-lg">{ticket.price}</p>
                        <button
                          onClick={() => handleBooking(ticket.type)}
                          className="mt-2 bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-200 transform hover:scale-105"
                        >
                          Select
                        </button>
                      </div>
                ...
                <div className="text-center">
                  <div className="flex items-center justify-center gap-2 mb-4">
                    ...
                    <span className="text-gray-600">
                      {bookingData.availableSeats} of {bookingData.totalSeats} seats remaining
                    </span>
                  </div>
